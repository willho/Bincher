...
- **Discovery Auto-Paper-Trading** (`server/discovery-paper-trading.ts`): Auto-opens paper trades on high-scoring tokens to learn which setups work. Two trigger modes: batch-triggered (30min scoring cycle, 70+ Pincher score, 2x TP, 35% SL, swing bias) and event-triggered (from discovery event bus, 60+ score, 1.8x TP, 30% SL, ~1min reaction). Token qualification filters: min $10k liquidity, 2h+ age, not crashing (>-30% 1h, >-50% 24h), volume >$1k. Tracks reaction speed per trade via `reactionSpeedMs` field. Limits: 10 trades/day, 20 max open, 4h mint cooldown. Event handlers for signal_buy, whale_activity, multi_signal_convergence, price_surge, plus social+trending combo.
