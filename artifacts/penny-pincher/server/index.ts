...
(async () => {
  ...
  try {
    ...
    // Can be bypassed for single-server / Replit deployments via SKIP_STARTUP_WIZARD=true
    if (process.env.SKIP_STARTUP_WIZARD !== "true") {
      ...
      console.log("⚠ Startup wizard skipped (SKIP_STARTUP_WIZARD=true) — running in single-server mode");
    }
